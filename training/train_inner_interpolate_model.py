...
logging.basicConfig(
    filename=os.path.join(LOGFILES_FOLDER, "inner_interpolate_model.log"),
    filemode="w",
    level=logging.INFO,
)
...
if __name__ == "__main__":
    ...
    # --- Inputs
    inputs = read_inputs_yaml(os.path.join("input_files", "inner_interpolate_model.yaml"))
    ...
    if args.use_wandb:
        wandb.init(project="inner_interpolate_model", entity="sudarshanvj")
        wandb.config.update(
            {
    ...
    train_dataset = InnerInterpolateDataset(
    ...
    validate_dataset = InnerInterpolateDataset(
    ...
    torch.save(model.state_dict(), os.path.join("model_files", "inner_interpolate_model.pt"))
