...
def test_input_dataset(dataset_options_factory):
    ...
    assert dataset.irreps_in == "4x0e+3x1o"
    ...
    assert dataset.product_tag == "product"
    assert dataset.transition_state_tag == "transition_state"
    assert dataset.irreps_in == "4x0e+3x1o+1x2e"


def test_input_dataset_multiple_eigenval(dataset_options_factory):
    dataset_options = dataset_options_factory("minimal", idx_eigenvalue=[0, -1])
    dataset = ReactionDataset(**dataset_options)
    assert dataset.max_s_functions == 4
    assert dataset.max_p_functions == 3
    assert dataset.max_d_functions == 0
    assert np.allclose(dataset.idx_eigenvalue, np.array([0, -1]))
    assert dataset.reactant_tag == "reactant"
    assert dataset.product_tag == "product"
    assert dataset.transition_state_tag == "transition_state"
    assert dataset.irreps_in == "4x0e+3x1o+4x0e+3x1o"
    dataset_options = dataset_options_factory("full", idx_eigenvalue=[0, -1])
    dataset = ReactionDataset(**dataset_options)
    assert dataset.max_s_functions == 4
    assert dataset.max_p_functions == 3
    assert dataset.max_d_functions == 1
    ...
    assert dataset.product_tag == "product"
    assert dataset.transition_state_tag == "transition_state"
    assert dataset.irreps_in == "4x0e+3x1o+1x2e+4x0e+3x1o+1x2e"
...
def test_output_dataset_minimal_basis_multiple_eigenval(dataset_options_factory):
    ...
    loader = DataLoader(dataset, batch_size=1, shuffle=False)
    for data in loader:
        # 12 atoms with 4 s functions and 3 p functions for two eigenvalues
        num_atoms = 12
        assert data.x.shape == (num_atoms, 26)
        assert data.x_final_state.shape == (num_atoms, 26)
        assert data.x_transition_state.shape == (num_atoms, 26)
        assert data.pos.shape == (num_atoms, 3)
        assert data.pos_final_state.shape == (num_atoms, 3)
        assert data.pos_transition_state.shape == (num_atoms, 3)
        assert data.species.shape == (num_atoms, 1)
...
def test_output_dataset_full_basis_multiple_eigenval(dataset_options_factory):
    ...
    for data in loader:
        # 12 atoms with 4 s functions and 3 p functions for two eigenvalues
        num_atoms = 12
        assert data.x.shape == (num_atoms, 36)
        assert data.x_final_state.shape == (num_atoms, 36)
        assert data.x_transition_state.shape == (num_atoms, 36)
        assert data.pos.shape == (num_atoms, 3)
        assert data.pos_final_state.shape == (num_atoms, 3)
        assert data.pos_transition_state.shape == (num_atoms, 3)
        assert data.species.shape == (num_atoms, 1)
...
def test_equivariance_dataset_minimal_basis_multiple_eigenval(
    dataset_options_factory, get_mapping_idx_to_euler_angles
):
    dataset_options = dataset_options_factory("minimal", idx_eigenvalue=[0, 1])
    dataset = ReactionDataset(**dataset_options)
    loader = DataLoader(dataset, batch_size=1, shuffle=False)
    idx_to_euler_angle = get_mapping_idx_to_euler_angles
    node_irreps = o3.Irreps("4x0e+3x1o+4x0e+3x1o")
    for _idx, data in enumerate(loader):
        x = data.x
        x_final_state = data.x_final_state
        x_transition_state = data.x_transition_state
...
def test_equivariance_dataset_full_basis_multiple_eigenval(
    dataset_options_factory, get_mapping_idx_to_euler_angles
):
    dataset_options = dataset_options_factory("full", idx_eigenvalue=[0, 1])
    dataset = ReactionDataset(**dataset_options)
    loader = DataLoader(dataset, batch_size=1, shuffle=False)
    idx_to_euler_angle = get_mapping_idx_to_euler_angles
    node_irreps = o3.Irreps("4x0e+3x1o+1x2e+4x0e+3x1o+1x2e")
    for _idx, data in enumerate(loader):
        x = data.x
        x_final_state = data.x_final_state
        x_transition_state = data.x_transition_state
