...
class Graph2PropertyModel(torch.nn.Module):
    ...
    def forward(self, x, x_reactant, edge_index, edge_attr, u, u_reactant, batch):
        ...
        out_reactant = torch.cat([u_reactant, averaged_node_features_reactant], dim=1)
        out_reactant = out_reactant.sum(dim=1)
...
class MessagePassingInterpolateDiffModel(torch.nn.Module):
    def __init__(
        self,
